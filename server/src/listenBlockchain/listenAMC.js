...
const { depositCoinUSDTBEP20, eventBuyToken } = require('./addDataBlockchain')
const linkRPC = "https://node1.amchain.net"

// const web3 = new Web3(new Web3.providers.WebsocketProvider(
//     linkRPC, // <-- Put your projectID here
//     {
//         clientConfig: {
...
//     }
// ))
const rpcTestnet = `https://bsc-testnet.drpc.org/`
const web3 = new Web3(`https://node1.amchain.net`)
const web3Http = new Web3(`https://node1.amchain.net`)
const networkId = process.env.START == 999999
...
const { eventBuyTokenAMC } = require('./addDataBlockchainAMC')
const { getRowToTable, updateRowToTable } = require('../query/funcQuery')
const { depositCoinAMC } = require('./addDataBlockchainAMC')
const { depositCoinUSDTAMC20 } = require('./addDataBlockchainAMC')
...
async function saveConfig(blockNew) {
    ...
    await updateRowToTable(`tb_block`,`block=${blockNew}`,`network='AMC'`)
    // console.log("save",lastBlock)
}
async function getEventContractAMC() {
 try {
    const config = await getRowToTable(`tb_block`, `network='AMC'`)
    var lastBlock = config[0].block

    var toBlock = await web3Http.eth.getBlockNumber() * 1
    // console.log(toBlock, ".....");
    if (toBlock - lastBlock > 5) {
        toBlock = lastBlock * 1 + 5
    }
    console.log("toBlock", toBlock)
    console.log("lastBlock", lastBlock)
    const arrayPromise = []

    for (let i = lastBlock; i <= toBlock; i++) {
        const block = await web3Http.eth.getBlock(i, true);
    ...
    usdtInstance.getPastEvents(`Transfer`, { fromBlock: lastBlock, toBlock: toBlock }
        , (err, result) => {
            if (err) {
                console.log(err);
                return
            ...
            if (result) {
                for (item of result) {
                    ...
                    arrayPromise.push(depositCoinUSDTAMC20(item))
                }
            }
            // arrayPromise.push(depositCoinUSDTBEP20(result))
            // const arrayPromise = []
            // addDataApi(err,result,arrayPromise,`transfer`)
            // await Promise.all(arrayPromise)
        })
    await Promise.all(arrayPromise)
    // lastBlock = toBlock + 1
    saveConfig(toBlock + 1)
 }catch(error){
 console.log(error,"Listien AMC")
 }
}
module.exports = {
    getEventContractAMC,
