...
export default function Header2({ history }) {
  ...
  const [callApiLoginWalletStatus, setCallApiLoginWalletStatus] = useState(
    api_status.pending
  );
  const renderAccountInfoContent = () =>
    callApiLoginWalletStatus !== api_status.fetching ? "" : "--d-none";
  ...
  const [callApiAddWalletStatus, setCallApiAddWalletStatus] = useState(
    api_status.pending
  );
  ...
  const callApiLoginWallet = async () => {
    try {
      ...
      const allWallet = [
        resp.data.data.infoUserLogin,
        ...resp.data.data.wallet,
      ];
      const currentUsdtBalance = allWallet.find(
        (item) => item.id === +userId
      )?.USDT_balance;
  ...
  const renderName = (item) => {
    if (item.nickName === null) {
      ...
      return (
        <>
          {item.nickName}{" "}
          <span style={{ opacity: 0.3 }}>({item.username})</span>
        </>
      );
    }
  };
  ...
  const renderAccountList = () => {
    ...
    const setActive = (item) => {
      return username === item.nickName || username === item.username
        ? "active"
        : "";
    };
    return listWallet.map((item, index) => {
      return (
        <div
          onClick={accountItemCLickHandle.bind(null, item)}
          key={index}
  ...
  const [fetchApiChangeNickNameStatus, setFetchApiChangeNickNameStatus] =
  ...
  const isButtonEditShow =
    editUsername?.id === editUsername?.parentUserIdWallet ? "--d-none" : "";
  ...
  const [register, changeNicknameSubmit, errors, reset] = useForm(
    ...
    <>
      ...
      >
        <ul className="header2__LanguageModal__content">
          ...
          <div className="header2__accountInfo">
            <div
              className={`header2__accountInfoContent ${renderAccountInfoContent()}`}
            >
              ...
              <div>
                {formatNumber(currentWalletUsdtBalance, i18n.language, 8)} USDT
              </div>
            </div>
      ...
      >
        <ul className="header2__LanguageModal__content">
          ...
          <div className="header2__accountList">{renderAccountList()}</div>
          <div className="header2__accountList__action">
            <Button
              loading={callApiAddWalletStatus === api_status.fetching}
              onClick={addMoreAccountCLickHandle}
            >
              Add more account
            </Button>
          </div>
      ...
      >
        <ul className="header2__LanguageModal__content">
          ...
          <div className="header2__accountDetail">
            <div
              className={`d-flex alignItem-c justify-sb gap-2 ${renderEditEnableClass(
                !editEnable
              )}`}
            >
              <div>{editUsername?.name}</div>
              <Button
            ...
            <form
              ...
              className={`d-flex alignItem-start justify-sb gap-2 ${renderEditEnableClass(
                editEnable
              )}`}
            >
              <Input
